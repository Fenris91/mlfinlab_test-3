...
def test_download_funding_rates_returns_correct_columns(monkeypatch, sample_rows):
    exchange = DummyExchange(pages=[sample_rows])
    _install_ccxt(monkeypatch, exchange)

    df = fr.download_funding_rates("BTC/USDT:USDT", since="2024-01-01")
...
def test_download_all_universe_sequential_with_progress(monkeypatch, tmp_path):
    ...
    def fake_download(symbol, since=None, limit=1000, exchange_id="binance"):
        ...
        return pd.DataFrame({"symbol": [fr._normalize_symbol(symbol)], "funding_rate": [0.1], "mark_price": [1]}, index=idx)

    monkeypatch.setattr(fr, "download_funding_rates", fake_download)
    stats = fr.download_all_universe(["BTC/USDT", "ETH/USDT"], cache_dir=str(tmp_path))
...
def test_build_funding_features_forward_fill():
    ...
    idx_o = pd.date_range("2024-01-01", periods=16, freq="1h", tz="UTC")
    funding = pd.DataFrame({"funding_rate": [0.1, 0.2]}, index=idx_f)
    ohlcv = pd.DataFrame({"close": range(16)}, index=idx_o)

    out = fr.build_funding_features(funding, ohlcv)

    assert out.loc[idx_o[1:8], "funding_rate"].isna().sum() == 0
...
def test_build_funding_features_positive_streak():
    ...
    funding = pd.DataFrame({"funding_rate": [0.1, 0.2, -0.1, 0.3, 0.4]}, index=idx)

    out = fr.build_funding_features(funding, pd.DataFrame(index=idx))

    assert out["funding_positive_streak"].tolist() == [1, 2, 0, 1, 2]


def test_build_funding_features_flip_detection():
    idx = pd.date_range("2024-01-01", periods=5, freq="1h", tz="UTC")
    funding = pd.DataFrame({"funding_rate": [0.1, -0.2, -0.3, 0.1, 0.2]}, index=idx)

    out = fr.build_funding_features(funding, pd.DataFrame(index=idx))

    assert out["funding_flip"].tolist() == [0, 1, 0, 1, 0]
...
def test_cli_download(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["funding_rates", "download", "--symbols", "BTC/USDT", "--cache-dir", str(tmp_path)],
    )
...
def test_cli_download_universe(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["funding_rates", "download-universe", "--config", str(cfg), "--cache-dir", str(tmp_path)],
    )
